...
import { Alert, Animated, Easing, Image, Modal, PermissionsAndroid, Platform, Pressable, StyleSheet, Switch, Text, TextInput, View } from 'react-native';
...
type Route = 'home'|'login'|'addDevice'|'deviceDetail'|'mapTool'|'profile'|'account'|'families'|'familyDetail'|'notifications'|'notificationSettings'|'help'|'operationHelp'|'article'|'about'|'general'|'text';
type ScheduleTime = { hour: number; minute: number; period: 'AM' | 'PM' };
type MapPoint = { x: number; y: number; yaw: number };
type MapCrcMode = 'protocol-table' | 'usb' | 'modbus' | 'xmodem' | 'xmodem-zero' | 'ccitt-false';
...
function MainApp({ onLogout, token, initialProfile }: { onLogout: () => Promise<void>; token: string | null; initialProfile: User | null }) {
  ...
  if (route === 'deviceDetail') return <DeviceDetail device={selectedDevice || devices[0]} onBack={close} onOpenMapTool={(d)=>{setSelectedDevice(d); setRoute('mapTool');}} onUpdated={async(d)=>{setSelectedDevice(d); await reload();}} />;
  if (route === 'mapTool') return <RobotMapTool device={selectedDevice || devices[0]} onBack={()=>setRoute('deviceDetail')} />;
...
function DeviceDetail({device,onBack,onOpenMapTool,onUpdated}:{device?:Device;onBack:()=>void;onOpenMapTool:(d:Device)=>void;onUpdated:(d:Device)=>void}) {
  ...
  return <Screen title="Device" onBack={onBack} onClose={onBack} right={<Pressable onPress={refresh} disabled={loading} hitSlop={10}><Feather name="refresh-cw" size={24} color={colors.green}/></Pressable>}>
    ...
    <View style={s.commandGrid}>
      ...
      <DeviceCommand icon="map-marker-path" label="Map Tool" onPress={()=>onOpenMapTool(current)} disabled={loading}/>
    </View>
  </Screen>
}
...
const MOWER_SERVICE_UUID = '0000fff0-0000-1000-8000-00805f9b34fb';
const MOWER_READ_UUID = '0000fff1-0000-1000-8000-00805f9b34fb';
const MOWER_WRITE_UUID = '0000fff2-0000-1000-8000-00805f9b34fb';
const MAP_STORAGE_KEY = 'mygardenos_mobile_map_tool_state';
...
function asciiToBase64(input: string) {
  const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';
  let output = '';
  for (let i = 0; i < input.length; i += 3) {
    const a = input.charCodeAt(i) & 255;
    const b = i + 1 < input.length ? input.charCodeAt(i + 1) & 255 : NaN;
    const c = i + 2 < input.length ? input.charCodeAt(i + 2) & 255 : NaN;
    output += alphabet[a >> 2];
...
function mapPointValue(value: number | string) {
  return Number.parseFloat(String(value)).toFixed(2).replace(/\.?0+$/, (match) => match === '.00' ? '' : match);
}

function crc16Usb(value: string) {
  let crc = 0xffff;
  for (let i = 0; i < value.length; i += 1) {
    crc ^= value.charCodeAt(i) & 255;
...
function crc16Modbus(value: string) {
  let crc = 0xffff;
  for (let i = 0; i < value.length; i += 1) {
    crc ^= value.charCodeAt(i) & 255;
    for (let bit = 0; bit < 8; bit += 1) {
...
function crc16ProtocolTable(value: string) {
  const crc = crc16Modbus(value);
  return ((crc & 255) << 8) | (crc >>> 8);
}

function crc16Xmodem(value: string, appendZeros = false, init = 0) {
  let crc = init;
  const bytes = value.split('').map((char) => char.charCodeAt(0) & 255);
...
function mapCrc(value: string, mode: MapCrcMode) {
  if (mode === 'protocol-table') return crc16ProtocolTable(value);
  if (mode === 'modbus') return crc16Modbus(value);
  if (mode === 'xmodem') return crc16Xmodem(value);
  if (mode === 'xmodem-zero') return crc16Xmodem(value, true);
  if (mode === 'ccitt-false') return crc16Xmodem(value, false, 0xffff);
  return crc16Usb(value);
}

function robotAckKey(commandValue: string) {
...
function RobotMapTool({device,onBack}:{device?:Device;onBack:()=>void}) {
  const managerRef = useRef<any>(null);
  const bleDeviceRef = useRef<any>(null);
  const notifySubRef = useRef<any>(null);
  const pendingAckRef = useRef<{ key:string; resolve:(value:any)=>void; timer:ReturnType<typeof setTimeout> } | null>(null);
  const holdTimerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const latestHeartbeatRef = useRef<MapPoint | null>(null);
  const recordingRef = useRef(false);
  const recordMinDistanceRef = useRef('0.10');
  const [bleReady,setBleReady]=useState(false);
  const [connected,setConnected]=useState(false);
  const [activeTab,setActiveTab]=useState<'remote'|'record'|'map'>('remote');
  const [status,setStatus]=useState('Disconnected');
  const [logs,setLogs]=useState<string[]>([]);
  const [dockPose,setDockPose]=useState('0.21,0.52,-33.1');
  const [mapId,setMapId]=useState('90191082715388544');
  const [newMapId,setNewMapId]=useState('90191082715388545');
  const [areaId,setAreaId]=useState('1');
  const [leaveDockDistance,setLeaveDockDistance]=useState('1.00');
  const [recordMinDistance,setRecordMinDistance]=useState('0.10');
  const [pointsPerPacket,setPointsPerPacket]=useState('10');
  const [commandDelay,setCommandDelay]=useState('900');
  const [crcMode,setCrcMode]=useState<MapCrcMode>('protocol-table');
  const [bleWriteMode,setBleWriteMode]=useState<'whole'|'chunked'>('whole');
  const [chunkConfig,setChunkConfig]=useState('20,35');
  const [recording,setRecording]=useState(false);
  const [recordedPoints,setRecordedPoints]=useState<MapPoint[]>([]);
  const [mappingStartPoint,setMappingStartPoint]=useState<MapPoint | null>(null);
  const [latestHeartbeat,setLatestHeartbeat]=useState<MapPoint | null>(null);
  const [mapCommands,setMapCommands]=useState('');
  const [capturedCommands,setCapturedCommands]=useState('');
  const appendLog = (line:string) => setLogs((prev)=>[`[${new Date().toLocaleTimeString()}] ${line}`,...prev].slice(0,160));

  useEffect(()=>{
    const BleManagerCtor = getBleManagerCtor();
    if (!BleManagerCtor) {
      setStatus('BLE native module unavailable. Build a dev client with react-native-ble-plx.');
      return;
    }
    managerRef.current = new BleManagerCtor();
    setBleReady(true);
    AsyncStorage.getItem(MAP_STORAGE_KEY).then((raw)=>{
      if (!raw) return;
      const saved = JSON.parse(raw);
      if (saved.dockPose) setDockPose(saved.dockPose);
      if (saved.mapId) setMapId(saved.mapId);
      if (saved.newMapId) setNewMapId(saved.newMapId);
      if (saved.areaId) setAreaId(saved.areaId);
      if (saved.mapCommands) setMapCommands(saved.mapCommands);
      if (saved.capturedCommands) setCapturedCommands(saved.capturedCommands);
      if (Array.isArray(saved.recordedPoints)) setRecordedPoints(saved.recordedPoints);
      if (saved.mappingStartPoint) setMappingStartPoint(saved.mappingStartPoint);
    }).catch(()=>{});
    return ()=>{
  ...
  useEffect(()=>{
    AsyncStorage.setItem(MAP_STORAGE_KEY, JSON.stringify({dockPose,mapId,newMapId,areaId,mapCommands,capturedCommands,recordedPoints,mappingStartPoint})).catch(()=>{});
  },[dockPose,mapId,newMapId,areaId,mapCommands,capturedCommands,recordedPoints,mappingStartPoint]);
  useEffect(()=>{ recordingRef.current = recording; },[recording]);
  useEffect(()=>{ recordMinDistanceRef.current = recordMinDistance; },[recordMinDistance]);
  ...
  const parseRobotResponse = (text:string) => {
    ...
    try {
      ...
      if (pending && robotAckKey(response.command || '') === pending.key) {
      ...
      if ((response.command === '$HEARTBEAT' || response.command === '$HEARTBE') && response.code === '00') {
        ...
        const point = {x:Number.parseFloat(parts[0]), y:Number.parseFloat(parts[1]), yaw:Number.parseFloat(parts[2])};
        if (Number.isFinite(point.x) && Number.isFinite(point.y)) {
          latestHeartbeatRef.current = point;
          setLatestHeartbeat(point);
          setRecordedPoints((prev)=>{
            if (!recordingRef.current) return prev;
            const minDistance = Math.max(0, Number.parseFloat(recordMinDistanceRef.current) || 0);
            const last = prev[prev.length - 1];
            if (last && distanceBetween(last, point) < minDistance) return prev;
            return [...prev, point];
          });
        }
      }
      if (response.command === '$STATUS' && response.code === '00') {
        const parts = String(response.data || '').split(',');
        if (parts[7]) setMapId(parts[7]);
      }
      if (response.command?.startsWith('$TASK,0') && response.code === '00') {
        const parts = String(response.data || '').split(',');
        if (parts[1]) setMapId(parts[1]);
      }
      if (response.command?.startsWith('$MAP,')) {
        setCapturedCommands((prev)=>{
          if (prev.split('\n').map((line)=>line.trim()).includes(response.command)) return prev;
          return [prev.trim(), response.command].filter(Boolean).join('\n');
        });
      }
    } catch {}
  };
  ...
  const connectBle = async () => {
    if (!bleReady || !managerRef.current) {
      Alert.alert('BLE unavailable', 'Install a development build with react-native-ble-plx; Expo Go cannot use this robot BLE link.');
      return;
    }
    const ok = await requestBlePermissions();
    if (!ok) {
      Alert.alert('Permission required', 'Bluetooth permission is required to connect to the mower.');
      return;
    }
    setStatus('Scanning NBMower...');
    appendLog('Scanning NBMower...');
    managerRef.current.startDeviceScan(null, null, async (error:any, scanned:any) => {
      if (error) {
        setStatus(error.message || 'BLE scan failed');
        appendLog(`ERROR ${error.message || error}`);
        managerRef.current?.stopDeviceScan?.();
        return;
      }
      if (!scanned || (scanned.name !== 'NBMower' && scanned.localName !== 'NBMower')) return;
      managerRef.current.stopDeviceScan();
      try {
        setStatus(`Connecting ${scanned.name || scanned.localName}...`);
        const connectedDevice = await scanned.connect();
        const readyDevice = await connectedDevice.discoverAllServicesAndCharacteristics();
        bleDeviceRef.current = readyDevice;
        notifySubRef.current?.remove?.();
        notifySubRef.current = readyDevice.monitorCharacteristicForService(MOWER_SERVICE_UUID, MOWER_READ_UUID, (notifyError:any, characteristic:any)=>{
          if (notifyError) {
            appendLog(`NOTIFY ERROR ${notifyError.message || notifyError}`);
            return;
          }
          parseRobotResponse(base64ToAscii(characteristic?.value || ''));
        });
        setConnected(true);
        setStatus('Connected and notifications enabled');
        appendLog('Connected and notifications enabled');
      } catch (connectError:any) {
        setConnected(false);
        setStatus(connectError?.message || 'Connect failed');
        appendLog(`ERROR ${connectError?.message || connectError}`);
      }
    });
  };
  ...
  const disconnectBle = async () => {
    ...
    if (bleDeviceRef.current?.id) await managerRef.current?.cancelDeviceConnection?.(bleDeviceRef.current.id).catch(()=>{});
    bleDeviceRef.current = null;
    setConnected(false);
    setStatus('Disconnected');
  };

  const writeRobotCommand = async (commandValue:string, label = 'TX', options?:{chunked?:boolean;chunkSize?:number;chunkDelayMs?:number}) => {
    if (!bleDeviceRef.current) throw new Error('Robot is not connected');
    const writeOne = (value:string) => bleDeviceRef.current.writeCharacteristicWithoutResponseForService(MOWER_SERVICE_UUID, MOWER_WRITE_UUID, asciiToBase64(JSON.stringify({command:value})));
    if (options?.chunked) {
      const payload = JSON.stringify({command:commandValue});
      const chunkSize = Math.max(1, options.chunkSize || 20);
      for (let i = 0; i < payload.length; i += chunkSize) {
        const chunk = payload.slice(i, i + chunkSize);
        await bleDeviceRef.current.writeCharacteristicWithoutResponseForService(MOWER_SERVICE_UUID, MOWER_WRITE_UUID, asciiToBase64(chunk));
        if (options.chunkDelayMs) await sleep(options.chunkDelayMs);
      }
      appendLog(`${label} ${commandValue} chunked ${payload.length} chars / ${chunkSize}`);
      return;
    }
    await writeOne(commandValue);
    appendLog(`${label} ${commandValue}`);
  };

  const sendCommandAndWait = async (commandValue:string, timeoutMs = 6000, options?:{chunked?:boolean;chunkSize?:number;chunkDelayMs?:number}) => {
    const key = robotAckKey(commandValue);
    return new Promise<any>(async (resolve, reject)=>{
      const timer = setTimeout(()=> {
        if (pendingAckRef.current?.key === key) pendingAckRef.current = null;
        reject(new Error(`Timed out waiting for ${key}`));
      }, timeoutMs);
      pendingAckRef.current = {key, resolve, timer};
      try {
        await writeRobotCommand(commandValue, 'Map', options);
      } catch (error) {
        clearTimeout(timer);
        pendingAckRef.current = null;
        reject(error);
  ...
  const sendSimpleCommand = async (commandValue:string) => {
    try {
      await writeRobotCommand(commandValue);
    } catch (e:any) {
      appendLog(`ERROR ${e?.message || e}`);
    }
  };

  const holdControl = (value:string) => {
    if (holdTimerRef.current) clearInterval(holdTimerRef.current);
    sendSimpleCommand(`$CONTROL,${value}`);
    holdTimerRef.current = setInterval(()=>sendSimpleCommand(`$CONTROL,${value}`), 280);
  };

  const stopControl = () => {
    if (holdTimerRef.current) clearInterval(holdTimerRef.current);
    holdTimerRef.current = null;
    sendSimpleCommand('$CONTROL,0');
  };

  const captureDock = () => {
    const point = latestHeartbeatRef.current;
    if (!point) {
      Alert.alert('No heartbeat yet', 'Connect the robot and wait for a heartbeat first.');
      return;
    }
    setDockPose(`${mapPointValue(point.x)},${mapPointValue(point.y)},${Number.isFinite(point.yaw) ? point.yaw.toFixed(1) : '0'}`);
  };

  const poseAhead = (point:MapPoint, distance:number) => {
    const radians = (Number.isFinite(point.yaw) ? point.yaw : 0) * Math.PI / 180;
    return {x: point.x + Math.cos(radians) * distance, y: point.y + Math.sin(radians) * distance, yaw: point.yaw};
  };

  const driveOutFromDock = async () => {
    try {
      await writeRobotCommand('$ACTION,0', 'Remote');
      await sleep(1200);
      const start = latestHeartbeatRef.current ? {...latestHeartbeatRef.current} : null;
      const distance = Math.max(0, Number.parseFloat(leaveDockDistance) || 1);
      const deadline = Date.now() + Math.max(1800, Math.ceil(distance / 0.4 * 1000) + 1200);
      while (Date.now() < deadline) {
        await writeRobotCommand('$CONTROL,1', `Drive out ${distance}m`);
        if (start && latestHeartbeatRef.current && distanceBetween(start, latestHeartbeatRef.current) >= distance) break;
        await sleep(300);
      }
      await writeRobotCommand('$CONTROL,0', 'Stop');
      if (latestHeartbeatRef.current) setMappingStartPoint({...latestHeartbeatRef.current});
      setRecording(true);
    } catch (e:any) {
      appendLog(`ERROR ${e?.message || e}`);
      stopControl();
  ...
  const buildManualMapCommands = (mode = crcMode) => {
    ...
    const closeThreshold = Math.max(0.2, Number.parseFloat(recordMinDistance) || 0.1);
    if (first && last && distanceBetween(first, last) > closeThreshold) boundaryPoints.push({...first});
    const mapIdValue = newMapId.trim() || mapId.trim() || String(Date.now());
    const [dockX='0', dockY='0', dockYaw='0'] = dockPose.split(',').map((part)=>part.trim());
    const normalizedAreaId = String(Math.max(1, Math.min(10, Number.parseInt(areaId, 10) || 1)));
    const areaFlags = Array(10).fill('0');
    areaFlags[Number(normalizedAreaId) - 1] = '1';
    const withCrc = (base:string) => `${base},${mapCrc(base, mode)}`;
    const commands = [withCrc(`$MAP,0,0,0,0,${mapIdValue},${dockX},${dockY},${dockYaw},${areaFlags.join(',')}`), '$ACTION,5,0'];
    const chunkSize = Math.max(1, Math.min(10, Number.parseInt(pointsPerPacket, 10) || 10));
    for (let index = 0; index < boundaryPoints.length; index += chunkSize) {
      ...
      commands.push(withCrc(`$MAP,${normalizedAreaId},1,${Math.floor(index / chunkSize)},${chunk.length},${coords}`));
    }
  ...
  const cloneCaptured = () => {
    const lines = capturedCommands.split('\n').map((line)=>line.trim()).filter(Boolean);
    if (!lines.length) {
      Alert.alert('No captured map', 'Paste or capture a working $MAP sequence first.');
      return;
    }
    const cloned = lines.map((line)=>{
      const parts = line.split(',');
      if (parts[0] === '$MAP' && parts[1] === '0' && parts[2] === '0' && parts.length > 5) {
        parts[5] = newMapId.trim() || `${mapId}_CLONE`;
        const withoutCrc = parts.slice(0, -1).join(',');
        return `${withoutCrc},${mapCrc(withoutCrc, crcMode)}`;
      }
      return line;
    });
    setMapCommands(cloned.join('\n'));
  };

  const sendMapSequence = async () => {
    const lines = mapCommands.split('\n').map((line)=>line.trim()).filter(Boolean);
    const delay = Math.max(0, Number.parseInt(commandDelay, 10) || 0);
    try {
      for (let index = 0; index < lines.length; index += 1) {
        const commandValue = lines[index].startsWith('{') ? JSON.parse(lines[index]).command : lines[index];
        if (commandValue.startsWith('$MAP,') || commandValue.startsWith('$ACTION,5')) {
          const [chunkSizeValue='20', chunkDelayValue='35'] = chunkConfig.split(',').map((part)=>part.trim());
          const response = await sendCommandAndWait(commandValue, bleWriteMode === 'chunked' ? 10000 : 6000, {
            chunked: bleWriteMode === 'chunked' && commandValue.startsWith('$MAP,'),
            chunkSize: Number.parseInt(chunkSizeValue, 10) || 20,
            chunkDelayMs: Number.parseInt(chunkDelayValue, 10) || 35,
          });
          appendLog(`ACK ${robotAckKey(commandValue)} ${response.code} ${response.message || ''}`);
          if (response.code !== '00') throw new Error(`${robotAckKey(commandValue)} failed: ${response.message || response.code}`);
        } else {
          await writeRobotCommand(commandValue, 'Map');
        }
        setStatus(`Sending map ${index + 1} / ${lines.length}`);
        if (index < lines.length - 1) await sleep(delay);
      }
      setStatus('Map sequence sent');
    } catch (e:any) {
  ...
  const readCurrentMap = async () => {
    await sendSimpleCommand('$STATUS');
    await sleep(350);
    await sendSimpleCommand('$TASK,0,0');
    await sleep(350);
    await sendSimpleCommand('$TASK,0,1');
  };

  const testHeader = async () => {
    const firstMap = mapCommands.split('\n').map((line)=>line.trim()).find((line)=>line.startsWith('$MAP,0,0'));
    if (!firstMap) {
      Alert.alert('No header', 'Generate map commands first.');
      return;
    }
    try {
      const response = await sendCommandAndWait(firstMap);
      setStatus(`Header ${response.code} ${response.message || ''}`);
    } catch (e:any) {
      setStatus(e?.message || String(e));
    }
  };

  const testAllCrcModes = async () => {
    const modes: MapCrcMode[] = ['protocol-table','usb','modbus','xmodem','xmodem-zero','ccitt-false'];
    for (const mode of modes) {
      try {
        const firstMap = buildManualMapCommands(mode).find((line)=>line.startsWith('$MAP,0,0'));
        if (!firstMap) continue;
        const response = await sendCommandAndWait(firstMap);
        appendLog(`CRC ${mode} -> ${response.code} ${response.message || ''}`);
        if (response.code === '00') {
          setCrcMode(mode);
          setStatus(`Header accepted with ${mode}`);
          return;
        }
      } catch (e:any) {
        appendLog(`CRC ${mode} -> ${e?.message || e}`);
      }
      await sleep(600);
    }
    setStatus('No CRC mode accepted');
  };

  const pointSummary = latestHeartbeat ? `${latestHeartbeat.x.toFixed(2)}, ${latestHeartbeat.y.toFixed(2)}, ${latestHeartbeat.yaw.toFixed(1)}` : '-';
  const canSend = connected && mapCommands.trim().length > 0;
  return <Screen title="Map Tool" onBack={onBack} onClose={onBack} right={<Pressable onPress={connected ? disconnectBle : connectBle} hitSlop={10}><Feather name={connected ? 'bluetooth' : 'bluetooth'} size={24} color={connected ? colors.green : colors.muted}/></Pressable>}>
    <View style={s.mapToolHeader}>
      <View style={s.detailMowerIcon}><MaterialCommunityIcons name="robot-mower" size={52} color="#5B6260"/></View>
      <View style={{flex:1,minWidth:0}}>
        <Text style={s.mapToolTitle} numberOfLines={1}>{device?.name || 'NBMower'}</Text>
        <Text style={s.mapToolMeta} numberOfLines={1}>{device?.serial || 'Select the robot, then connect BLE'}</Text>
        <Text style={s.mapToolStatus}>{status}</Text>
      </View>
    </View>

    <View style={s.mapTabs}>
      {(['remote','record','map'] as const).map((tab)=><Pressable key={tab} style={[s.mapTab, activeTab===tab && s.mapTabActive]} onPress={()=>setActiveTab(tab)}><Text style={[s.mapTabText, activeTab===tab && s.mapTabTextActive]}>{tab === 'remote' ? 'Remote' : tab === 'record' ? 'Record' : 'Map'}</Text></Pressable>)}
    </View>

    {activeTab === 'remote' && <Card>
      <Text style={s.mapSectionTitle}>BLE Control</Text>
      <View style={s.mapActionRow}>
        <Button title={connected ? 'Disconnect' : 'Connect NBMower'} onPress={connected ? disconnectBle : connectBle} variant={connected ? 'light' : 'green'} />
        <Button title="Read Map ID" onPress={readCurrentMap} variant="light" />
      </View>
      <View style={s.remotePad}>
        <ControlButton label="↖" value="6" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="↑" value="1" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="↗" value="7" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="←" value="3" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="■" value="0" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="→" value="4" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="↙" value="8" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="↓" value="2" onHold={holdControl} onStop={stopControl}/>
        <ControlButton label="↘" value="5" onHold={holdControl} onStop={stopControl}/>
      </View>
      <View style={s.mapActionRow}>
        <Button title="Remote Mode" onPress={()=>sendSimpleCommand('$ACTION,0')} variant="light" />
        <Button title="Standby" onPress={()=>sendSimpleCommand('$ACTION,6')} variant="light" />
      </View>
    </Card>}

    {activeTab === 'record' && <>
      <Card>
        <Text style={s.mapSectionTitle}>Manual Mapping Recorder</Text>
        <MapPreview points={recordedPoints} dockPose={dockPose} startPoint={mappingStartPoint}/>
        <Text style={s.mapHint}>Latest heartbeat: {pointSummary}</Text>
        <Text style={s.mapHint}>Points: {recordedPoints.length} · Recording: {recording ? 'Yes' : 'No'}</Text>
        <View style={s.mapActionRow}>
          <Button title="Use Dock Pose" onPress={captureDock} variant="light" />
          <Button title="Drive Out 1m" onPress={driveOutFromDock} variant="light" />
        </View>
        <View style={s.mapActionRow}>
          <Button title={recording ? 'Stop Recording' : 'Start Recording'} onPress={()=>setRecording((value)=>!value)} />
          <Button title="Clear" onPress={()=>{setRecordedPoints([]);setMappingStartPoint(null);}} variant="red" />
        </View>
        <Button title="Generate Map From Points" onPress={generateMap} variant="green" />
      </Card>
      <MapInputs dockPose={dockPose} setDockPose={setDockPose} mapId={mapId} setMapId={setMapId} newMapId={newMapId} setNewMapId={setNewMapId} areaId={areaId} setAreaId={setAreaId} leaveDockDistance={leaveDockDistance} setLeaveDockDistance={setLeaveDockDistance} recordMinDistance={recordMinDistance} setRecordMinDistance={setRecordMinDistance} pointsPerPacket={pointsPerPacket} setPointsPerPacket={setPointsPerPacket}/>
    </>}

    {activeTab === 'map' && <>
      <Card>
        <Text style={s.mapSectionTitle}>Generated / Captured Commands</Text>
        <View style={s.mapModeRow}>
          {(['protocol-table','usb','modbus','xmodem','xmodem-zero','ccitt-false'] as MapCrcMode[]).map((mode)=><Pressable key={mode} style={[s.modeChip, crcMode===mode && s.modeChipActive]} onPress={()=>setCrcMode(mode)}><Text style={[s.modeChipText, crcMode===mode && s.modeChipTextActive]}>{mode}</Text></Pressable>)}
        </View>
        <View style={s.mapActionRow}>
          <Button title="Generate" onPress={generateMap} variant="light" />
          <Button title="Clone Captured" onPress={cloneCaptured} variant="light" />
        </View>
        <TextInput style={[s.input,s.mapTextarea]} value={mapCommands} onChangeText={setMapCommands} multiline placeholder="$MAP / $ACTION sequence"/>
        <View style={s.mapActionRow}>
          <Button title="Test Header" onPress={testHeader} variant="light" />
          <Button title="Test CRC Modes" onPress={testAllCrcModes} variant="light" />
        </View>
        <View style={s.mapActionRow}>
          <Button title="Whole Write" onPress={()=>setBleWriteMode('whole')} variant={bleWriteMode === 'whole' ? 'green' : 'light'} />
          <Button title="Chunked Write" onPress={()=>setBleWriteMode('chunked')} variant={bleWriteMode === 'chunked' ? 'green' : 'light'} />
        </View>
        <View style={s.mapActionRow}>
          <LabeledInput label="Delay ms" value={commandDelay} onChangeText={setCommandDelay}/>
          <LabeledInput label="Chunk bytes,delay" value={chunkConfig} onChangeText={setChunkConfig}/>
        </View>
        <Button title="Send Map Sequence" onPress={sendMapSequence} variant={canSend ? 'green' : 'light'} />
      </Card>
      <Card>
        <Text style={s.mapSectionTitle}>Captured Working Map Commands</Text>
        <TextInput style={[s.input,s.mapTextareaSmall]} value={capturedCommands} onChangeText={setCapturedCommands} multiline placeholder="Paste native app / vendor $MAP commands here"/>
      </Card>
    </>}

    <Card>
      <Text style={s.mapSectionTitle}>BLE Log</Text>
      {logs.length === 0 ? <Text style={s.mapHint}>No log yet.</Text> : logs.map((line,index)=><Text key={`${line}-${index}`} style={s.logLine}>{line}</Text>)}
    </Card>
  </Screen>;
}
...
function ControlButton({label,value,onHold,onStop}:{label:string;value:string;onHold:(value:string)=>void;onStop:()=>void}) {
  return <Pressable style={s.controlButton} onPressIn={()=>onHold(value)} onPressOut={onStop}>
    <Text style={s.controlButtonText}>{label}</Text>
  </Pressable>;
}
...
function MapInputs(props:{dockPose:string;setDockPose:(v:string)=>void;mapId:string;setMapId:(v:string)=>void;newMapId:string;setNewMapId:(v:string)=>void;areaId:string;setAreaId:(v:string)=>void;leaveDockDistance:string;setLeaveDockDistance:(v:string)=>void;recordMinDistance:string;setRecordMinDistance:(v:string)=>void;pointsPerPacket:string;setPointsPerPacket:(v:string)=>void}) {
  return <Card>
    <Text style={s.mapSectionTitle}>Map Settings</Text>
    <LabeledInput label="Map ID" value={props.mapId} onChangeText={props.setMapId}/>
    <LabeledInput label="New Map ID" value={props.newMapId} onChangeText={props.setNewMapId}/>
    <LabeledInput label="Dock X,Y,Yaw" value={props.dockPose} onChangeText={props.setDockPose}/>
    <View style={s.mapActionRow}>
      <LabeledInput label="Area ID" value={props.areaId} onChangeText={props.setAreaId}/>
      <LabeledInput label="Points / packet" value={props.pointsPerPacket} onChangeText={props.setPointsPerPacket}/>
    </View>
    <View style={s.mapActionRow}>
      <LabeledInput label="Leave dock m" value={props.leaveDockDistance} onChangeText={props.setLeaveDockDistance}/>
      <LabeledInput label="Sample m" value={props.recordMinDistance} onChangeText={props.setRecordMinDistance}/>
    </View>
  </Card>;
}

function MapPreview({points,dockPose,startPoint}:{points:MapPoint[];dockPose:string;startPoint:MapPoint|null}) {
  const dockParts = dockPose.split(',').map((part)=>Number.parseFloat(part.trim()));
  const dock = Number.isFinite(dockParts[0]) && Number.isFinite(dockParts[1]) ? {x:dockParts[0], y:dockParts[1], yaw: dockParts[2] || 0} : null;
  const refs = [...points, ...(dock ? [dock] : []), ...(startPoint ? [startPoint] : [])];
  if (!refs.length) return <View style={s.mapPreview}><Text style={s.mapHint}>No recorded points yet</Text></View>;
  const xs = refs.map((point)=>point.x);
  const ys = refs.map((point)=>point.y);
  const minX = Math.min(...xs);
  const maxX = Math.max(...xs);
  const minY = Math.min(...ys);
  const maxY = Math.max(...ys);
  const toPct = (point:MapPoint) => ({
    left: `${8 + ((point.x - minX) / Math.max(0.5, maxX - minX)) * 84}%` as any,
    top: `${92 - ((point.y - minY) / Math.max(0.5, maxY - minY)) * 84}%` as any,
  });
  return <View style={s.mapPreview}>
    {points.map((point,index)=><View key={`${point.x}-${point.y}-${index}`} style={[s.mapDot, toPct(point), index === 0 && s.mapDotStart, index === points.length - 1 && s.mapDotEnd]}/>)}
    {dock && <View style={[s.mapDockDot, toPct(dock)]}/>}
    {startPoint && <View style={[s.mapStartDot, toPct(startPoint)]}/>}
  </View>;
}
